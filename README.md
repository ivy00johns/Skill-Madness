...
A multi-agent orchestration toolkit for Claude Code: **70 portable skills** that turn a one-line ask into a contract-first parallel build, run **autonomous loops** until your tests are actually green, and install into **eleven AI coding tools** from a single `SKILL.md` source.
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/skills-70-success.svg" alt="70 skills" />
...
> 🚪 **New here, or not sure which skill to reach for?** Type **`/madness`** in Claude Code and just say what you want. It reads your intent, picks the right starting skill out of all 70, confirms before anything expensive, and launches it — so you never have to memorize the catalog. One front door for the whole toolkit.
...
- 🪄 **One front door** — `madness` is the router: type `/madness`, describe the task, and it picks the right starting skill out of all 70 and launches it. The cure for "which skill was that again?" across a 70-skill library.
- 🪜 **Progressive disclosure** — frontmatter (~100 tokens) always loaded, body loaded on trigger, references loaded on demand. A 70-skill library stays cheap to host.
...
> - **The orchestrator + 70-skill library is the mature part.** All bodies under 500 lines, zero ownership conflicts, zero broken cross-references, full Ubuntu + macOS lint matrix on every push.
...
That installs all 70 skills into Claude Code's plugin storage. No clone, no symlink, no edits-to-the-repo workflow. Use this if you just want the skills.
...
flowchart TB
    ...
    subgraph workflows["⚙️ workflows/ — 33 skills"]
        direction TB
        pb[plan-builder]
        cm[context-manager]
...
70 skills organized into seven categories. All bodies under 500 lines, all frontmatter validated, zero ownership conflicts, zero broken cross-references.
...
| 38 | `diagnose-loop` | workflow | Build the fast feedback loop first, then debug |
| 39 | `architecture-rescue` | workflow | Find shallow modules + missing seams; deletion + two-adapter test |
| 40 | `caveman` | workflow | Ultra-compressed responses, ~75% token cut, full accuracy |
| 41 | `render-sanity` | workflow | "Tests pass but UI is broken" gate — click-through + stale-data scan |
| 42 | `design-token-guard` | workflow | Block inline styles / hardcoded CSS from bypassing the design-token system |
| 43 | `class-extraction-guard` | workflow | Catch utility-class soup — repeated inline class runs that should be extracted |
| 44 | `sync-skills` | workflow | Symlink/copy skills to `~/.claude/skills/` and Cursor |
| 45 | `settings-consolidator` | workflow | Merge Claude Code permissions across projects |
| 46 | `repo-deep-dive` | workflow | OSS repo → 12–14 doc technical reference series |
| 47 | `llm-wiki` | workflow | Bootstrap + maintain LLM-powered knowledge bases |
| 48 | `interactive-doc` | workflow | Paired Obsidian `.md` + self-contained `.html` companion |
| 49 | `ui-brief` | workflow | Opinionated UI design briefs (greenfield + rebuild) |
| 50 | `claude-design-brief` | workflow | Paste-ready prompts for Claude Design mockup canvas |
| 51 | `mermaid-charts` | workflow | Expert-quality diagrams (15–30+ node systems) |
| 52 | `nano-banana` | workflow | Google Gemini image generation + batch ops |
| 53 | `artifact-publish` | workflow | Publish a visual/interactive deliverable as a shareable, hosted claude.ai Artifact page |
| 54 | `playwright` | workflow | Browser-based E2E + screenshots with visible Chrome |
| 55 | `website-walkthrough-video` | workflow | Smooth scrolling walkthrough mp4 of a whole site (desktop + mobile) |
| 56 | `railway-deploy` | workflow | Deploy to Railway (Dockerfile, multi-service, GraphQL API) |
| 57 | `use-freellmapi` | workflow | Wire a project to FreeLLMAPI — a local free-model OpenAI-compatible proxy |
| 58 | `loop-controller` | loop | Foundation harness: 5-part contract + guardrail stack every loop composes on |
| 59 | `fix-until-green` | loop | Drive tests+lint+typecheck green without cheating the gate |
| 60 | `contract-conformance-loop` | loop | Build-until-spec: implement until contract criteria hold, fresh-context evaluator |
| 61 | `coverage-loop` | loop | Grow the test suite to a coverage target without gaming the metric |
| 62 | `perf-loop` | loop | Profile → optimize → re-benchmark a metric under budget, no regression |
| 63 | `migration-loop` | loop | Migrate an enumerated target set until done + suite green + no legacy pattern |
| 64 | `babysit` | loop | Scheduled review-and-revise: keep a PR rebased + green via `/loop` |
| 65 | `self-healing-loop` | loop | Watch logs/CI → root-cause → fix → verify → PR on a poll cadence |
| 66 | `nightly-docs-and-changelog` | loop | Nightly `/schedule` sweep keeping docs + changelog from rotting |
| 67 | `dependency-health-loop` | loop | Scheduled audit + gated update + green gate; HITL on majors |
| 68 | `codebase-exploration-loop` | loop | Fan-out read-only mappers until seed questions are answered |
| 69 | `orchestrator-task-loop` | loop | Outer loop draining the Agent Teams shared task board (experimental) |
| 70 | `repo-cleanup-loop` | loop | Weekly evidence-gated branch/PR/worktree hygiene, recover-before-delete |
...
│
├── skills/                           # the canonical skill library (70)
...
<details>
<summary><b>"My non-Claude-Code host doesn't see all 70 skills"</b></summary>
...
- [x] **Skill library** — 70 skills, seven categories, all linted
