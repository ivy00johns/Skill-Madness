...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/skills-69-success.svg" alt="69 skills" />
...
> - **The orchestrator + 69-skill library is the mature part.** All bodies under 500 lines, zero ownership conflicts, zero broken cross-references, full Ubuntu + macOS lint matrix on every push.
...
That installs all 69 skills into Claude Code's plugin storage. No clone, no symlink, no edits-to-the-repo workflow. Use this if you just want the skills.
...
69 skills organized into seven categories. All bodies under 500 lines, all frontmatter validated, zero ownership conflicts, zero broken cross-references.
...
| 53 | `playwright` | workflow | Browser-based E2E + screenshots with visible Chrome |
| 54 | `website-walkthrough-video` | workflow | Smooth scrolling walkthrough mp4 of a whole site (desktop + mobile) |
| 55 | `railway-deploy` | workflow | Deploy to Railway (Dockerfile, multi-service, GraphQL API) |
| 56 | `use-freellmapi` | workflow | Wire a project to FreeLLMAPI — a local free-model OpenAI-compatible proxy |
| 57 | `loop-controller` | loop | Foundation harness: 5-part contract + guardrail stack every loop composes on |
| 58 | `fix-until-green` | loop | Drive tests+lint+typecheck green without cheating the gate |
| 59 | `contract-conformance-loop` | loop | Build-until-spec: implement until contract criteria hold, fresh-context evaluator |
| 60 | `coverage-loop` | loop | Grow the test suite to a coverage target without gaming the metric |
| 61 | `perf-loop` | loop | Profile → optimize → re-benchmark a metric under budget, no regression |
| 62 | `migration-loop` | loop | Migrate an enumerated target set until done + suite green + no legacy pattern |
| 63 | `babysit` | loop | Scheduled review-and-revise: keep a PR rebased + green via `/loop` |
| 64 | `self-healing-loop` | loop | Watch logs/CI → root-cause → fix → verify → PR on a poll cadence |
| 65 | `nightly-docs-and-changelog` | loop | Nightly `/schedule` sweep keeping docs + changelog from rotting |
| 66 | `dependency-health-loop` | loop | Scheduled audit + gated update + green gate; HITL on majors |
| 67 | `codebase-exploration-loop` | loop | Fan-out read-only mappers until seed questions are answered |
| 68 | `orchestrator-task-loop` | loop | Outer loop draining the Agent Teams shared task board (experimental) |
| 69 | `repo-cleanup-loop` | loop | Weekly evidence-gated branch/PR/worktree hygiene, recover-before-delete |
...
<details>
<summary><b>"My non-Claude-Code host doesn't see all 69 skills"</b></summary>
...
- [x] **Skill library** — 69 skills, seven categories, all linted
