...
| **Declaring done without ux-review on UI builds** | Tests pass + dev server boots is not the bar for a UI project. After the build, invoke `ux-review` (or run non-headless Playwright + screenshots manually) and address what comes back. Visual quality is verifiable; verify it. |
| **Accepting a UI verdict that was inferred, not observed** | No agent may report a UI's design or function as validated from source reading, a headless run, curl, or "it should work." UI validation happens in a visible (non-headless) Playwright browser; anything else is a guess, and a guessed verdict doesn't clear a gate. |
| **Agents editing files they haven't read / guessing what code does** | Every file change must follow reading the actual file and its contracts. "Probably does X" is not a basis for an edit; open the file, read it, then edit. This applies to every role agent, not just reviewers. |
