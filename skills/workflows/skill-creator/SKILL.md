...
A skill for creating new skills and iteratively improving them using Hermes Agent.
...
Research in parallel via subagents if needed — come prepared with context to reduce burden on the user.
...
- **description**: When to trigger, what it does. This is the primary triggering mechanism — include both what the skill does AND specific contexts for when to use it. All "when to use" info goes here, not in the body. Make descriptions "pushy" — enumerate contexts where the skill should activate. Stay under 1024 characters.
...
Put results in `.workspaces/<skill-name>/` at the repo root. Within the workspace, organize results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Don't create all of this upfront — just create directories as you go.
...
When each subagent task completes, the task notification includes timing data. Save this data immediately to `timing.json` in the run directory:
...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   If there's no display available, use `--static <output_path>` to write a standalone HTML file instead of starting a server. Feedback will be downloaded as a `feedback.json` file when the user clicks "Submit All Reviews". After download, copy `feedback.json` into the workspace directory for the next iteration to pick up.
...
Please add steps to your todo list, if you have such a thing, to make sure you don't forget. Specifically put "Create evals JSON and run eval-viewer/generate_review.py so human can review test cases" in your todo list to make sure it happens.
