...
def _call_hermes(prompt: str, model: str | None = None, timeout: int = 300) -> str:
    """Run `hermes chat -q --oneshot -Q` with the prompt on stdin and return the text response.
    ...
    # Strip the session_id line from output
...
The description appears in Hermes' "available_skills" list. When a user sends a query, the agent decides whether to invoke the skill based solely on the title and on this description. Your goal is to write a description that triggers for relevant queries, and doesn't trigger for irrelevant ones.
